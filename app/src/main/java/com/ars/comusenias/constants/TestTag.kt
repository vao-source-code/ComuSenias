package com.ars.comusenias.constants

class TestTag {
    companion object {
        const val TAG_BUTTON_APP = "buttonapp"
        const val TAG_TEXT_CLICK_FOOTER = "textclickfooter"
        const val TAG_CHECKBOX_APP = "checkboxApp"
        const val TAG_FORGET_MY_PASS = "ForgetMyPass"
        const val TAG_TEXT_FIELD_APP = "textFieldApp"
        const val TAG_TEXT_APP = "textApp"
        const val TAG_TEXT_FIELD_APP_PASS = "textFieldAppPass"
        const val TAG_ICON_IS_HIDEN = "iconIsHiden"
        const val TAG_MESSAGE_ERROR = "messageError"
        const val TAG_ONBOARDING_ITEM = "onboardingitem"
        const val TAG_BUTTON_NEXT = "buttonNext"
        const val TAG_LOGIN_SCREEN = "loginScreen"
        const val TAG_INDICATOR = "indicator"
        const val TAG_BOX_SPLASH_SCREEN = "boxSplashScreen"
        const val TAG_COLUMN_SPLASH_SCREEN = "columnSplashScreen"
        const val TAG_BUTTON_DEFAULT = "columnSplashScreen"
        const val TAG_PROFESIONAL_SCREEN = "profesionalScreen"
        const val TAG_CONTENT_CARD_GAME = "contentCardGame"
        const val TAG_LINE_COLOR_CARD = "lineColor"
        const val TAG_MATCH_SIGN = "matchSign"

    }
}